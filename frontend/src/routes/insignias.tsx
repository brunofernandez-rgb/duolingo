...
function Insignias({ user }: { user: { id: string } }) {
  ...
  return <div className="space-y-5"><h1 className="text-2xl font-extrabold">{t("badges.title")}</h1>{query.isError ? <p>No se pudieron cargar las insignias.</p> : <div className="grid gap-3 sm:grid-cols-2">{(query.data ?? []).map((badge) => <article key={badge.insignia_id} className="rounded-2xl border-2 border-border bg-card p-5"><Award className="mb-3 text-gold" /><h2 className="font-extrabold">{badge.insignia_nombre}</h2><p className="text-sm font-bold text-muted-foreground">{badge.insignia_descripcion}</p><p className="mt-2 text-xs font-bold text-muted-foreground">{t("badges.unlocked")} {new Intl.DateTimeFormat(lang).format(new Date(badge.fecha))}</p></article>)}</div>}</div>;
}
