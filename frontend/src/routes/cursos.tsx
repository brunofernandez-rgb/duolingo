...
function Cursos({ user }: { user: { id: string } }) {
  ...
  if (cursosQuery.isLoading || idiomasQuery.isLoading || inscripcionesQuery.isLoading) return <p>{t("course.loading")}</p>;
  if (cursosQuery.isError || idiomasQuery.isError || inscripcionesQuery.isError) return <p>{t("course.loadError")}</p>;
    ...
    <div className="space-y-8">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-xl font-extrabold">
                            {curso.nivel === "TECNICO" ? levelName(lang, curso.nivel) : `${t("course.level")} ${curso.nivel}`}
                          </p>
      ...
      {cursosCompletados.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-xl font-extrabold">{t("course.completedCourses")}</h2>
          <div className="grid gap-3 sm:grid-cols-2">
            {cursosCompletados.map((curso) => (
              <article key={curso.id} className="flex items-center justify-between rounded-2xl border-2 border-success bg-success-soft p-4">
                ...
                <span className="font-extrabold text-success">{t("course.done")}</span>
              </article>
            ))}
          </div>
        </section>
