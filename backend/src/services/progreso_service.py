...
class ProgresoService:
    ...
    def registrar_intento(self, dto: CreateProgresoDTO) -> ProgresoResponseDTO | None:
        ...
        if progreso_existente:
            ...
            progreso_existente.fecha = ahora
            if es_completada and not ya_completada:
                progreso_existente.fecha_completada = ahora
                progreso_existente.xp_obtenida = leccion.xp_recompensa
            progreso = self.repo.update(progreso_existente)
        else:
            progreso = self.repo.create(
                dto.usuario_id,
                dto.leccion_id,
                dto.puntaje,
                es_completada,
                ahora,
                fecha_completada=ahora if es_completada else None,
                xp_obtenida=leccion.xp_recompensa if es_completada else 0,
            )
